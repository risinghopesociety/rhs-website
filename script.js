...
document.addEventListener("DOMContentLoaded", () => {
        ...
        .then(res => {
          ...
          if (res.success) {
            showRegAlert("status-green", `
              ...
              <p style="margin-top:8px">Your application is currently <strong>Under Process</strong>. Our team will review your application and contact you after approval.</p>
        ...
        if (res.success && res.found) {
          ...
          if (stLow === "active" || stLow === "expired" || stLow === "banned") {
            ...
            const msgBody = isRejected
              ? `Dear <strong>${m.fullName || "Applicant"}</strong>, Your Membership Request is currently <strong>${st}</strong>. Please contact us.`
              : `Dear <strong>${m.fullName || "Applicant"}</strong>, Your Membership Request is currently <strong>${st}</strong>. You will be contacted after approval.`;
            ...
            certResult.innerHTML = `
              <div class="status-msg ${vibe}">
                ...
                <div class="reg-alert-btns">
                  <button class="btn btn-ghost" onclick="window.closeLedger()">
  ...
  window.printCertificate = function (member) {
    ...
    pa.innerHTML = `
      <style>@page{margin:10mm;size:A4;}body{-webkit-print-color-adjust:exact;print-color-adjust:exact;margin:0}.no-print{display:none!important}</style>
      <div style="border:5px double #14534F;border-radius:6px;font-family:Georgia,serif;max-width:680px;margin:0 auto;overflow:hidden">
        ...
        <div style="padding:20px 28px;display:grid;grid-template-columns:1fr 1fr;gap:0;border-bottom:1px solid #E7DFD2">
          ${[["CNIC",member.cnic||"—"],["Date of Birth",member.dob||"—"],["Mobile",member.mobile||"—"],["Valid Upto",member.validUpto||"—"],["Gender",member.gender||"—"],["Province",member.province||"—"]].map(([l,v])=>`<div style="padding:10px 0;border-bottom:1px dashed #E7DFD2"><div style="font-size:9px;letter-spacing:.1em;color:#8A9A96;font-family:sans-serif;text-transform:uppercase">${l}</div><div style="font-size:13px;font-weight:600;color:#1C2B29;font-family:sans-serif;margin-top:2px">${v}</div></div>`).join("")}
          <div style="padding:10px 0;grid-column:1/-1;border-bottom:1px dashed #E7DFD2"><div style="font-size:9px;letter-spacing:.1em;color:#8A9A96;font-family:sans-serif;text-transform:uppercase">Address</div><div style="font-size:13px;font-weight:600;color:#1C2B29;font-family:sans-serif;margin-top:2px">${member.address||"—"}</div></div>
        </div>
        ...
        <div style="padding:11px 28px;background:#EEF8F1;text-align:center">
          <div style="font-size:9.5px;color:#4A6B60;font-family:sans-serif">✅ Verify: <strong>${verifyUrl}</strong> &nbsp;|&nbsp; 📞 ${window.NGO.phone} &nbsp;|&nbsp; 📧 ${window.NGO.email}</div>
  ...
  if (grantCnicEl) {
    grantCnicEl.addEventListener("blur", function () {
      ...
      grantCnicTimer = setTimeout(() => {
        RHS.checkGrantByCnic(val).then(res => {
          if (!res.found) return;
          const c = res.grant;
          ...
          if (grantResult) {
            ...
            grantResult.innerHTML = `
              <div class="status-msg status-yellow">
                ...
                <p>Dear <strong>${c.name}</strong>, Your case is already submitted as <strong>${c.crn}</strong>.</p>
                <div class="reg-alert-btns">
            ...
            if (grantResult) {
              ...
              grantResult.innerHTML = `
                <div class="status-msg status-green">
                  <i class="fa-solid fa-circle-check" style="font-size:2.5rem;color:#2E9E5B;display:block;margin-bottom:12px"></i>
                  <div class="status-title" style="color:#1A6B3A">Request Submitted!</div>
                  <p>${res.message || ""}</p>
                  <div class="reg-alert-btns">
                    <button class="btn btn-primary" onclick="window.hideHelpSection();setTimeout(()=>window.showHelpSection('status'),100)">Check Status</button>
                    <button class="btn btn-ghost" onclick="window.hideHelpSection()">Back</button>
                  </div>
                </div>`;
            }
          } else {
  ...
  function gsBack() { gsShowForm(); window.hideHelpSection(); }
      ...
      RHS.getGrantStatus(cnic, dob)
        .then(res => {
          ...
          if (!res.success || !res.grants || !res.grants.length) {
            gsShowResult(`<div class="status-msg status-red"><i class="fa-solid fa-circle-xmark"></i><div class="status-title">No Request Found</div><p>${res.message || "No grant request found."}</p><div class="reg-alert-btns" style="margin-top:16px"><button class="btn btn-primary" onclick="window._gsShowForm()"><i class="fa-solid fa-search"></i> Search Again</button><button class="btn btn-ghost" onclick="window._gsBack()"><i class="fa-solid fa-arrow-left"></i> Back</button></div></div>`);
            return;
          }
          ...
          for (const g of list) {
            ...
            if (s === "new") { msg = `Your Request <strong>${g.crn}</strong> received. Our team will contact you soon.<br>📞 <a href="tel:+92${window.NGO.alert.replace(/\D/g,'').slice(-10)}">${window.NGO.alert}</a>`; vibe = "status-yellow"; icon = "fa-hourglass-half"; }
            else if (s === "assigned") { msg = `Case assigned to <strong>${g.assignedTo||"Team Member"}</strong>${g.assignedContact?` (${g.assignedContact})`:""}. Please cooperate during verification.<br>📞 <a href="tel:+92${window.NGO.alert.replace(/\D/g,'').slice(-10)}">${window.NGO.alert}</a>`; vibe = "status-green"; icon = "fa-user-check"; }
            else if (s === "completed" && d !== "approved" && d !== "rejected") { msg = `Case <strong>${g.crn}</strong> verification completed. Please wait for decision.<br>📞 <a href="tel:+92${window.NGO.alert.replace(/\D/g,'').slice(-10)}">${window.NGO.alert}</a>`; vibe = "status-yellow"; icon = "fa-clipboard-check"; }
            else if (d === "approved" && s !== "closed") {
              html += `<div style="background:linear-gradient(135deg,#e8f8f0,#d0f0e0,#e8f8f0);border:2px solid #2E9E5B;border-radius:18px;padding:32px 28px 24px;text-align:center;margin-bottom:16px;box-shadow:0 4px 24px rgba(46,158,91,0.13);position:relative;overflow:hidden"><div style="position:absolute;top:12px;left:18px;font-size:1.3rem;opacity:.5">🎉</div><div style="position:absolute;top:12px;right:18px;font-size:1.3rem;opacity:.5">🎊</div><div style="width:72px;height:72px;border-radius:50%;background:linear-gradient(135deg,#2E9E5B,#4CAF8A);display:flex;align-items:center;justify-content:center;margin:0 auto 16px;box-shadow:0 4px 16px rgba(46,158,91,0.35)"><i class="fa-solid fa-circle-check" style="font-size:2rem;color:#fff"></i></div><div style="font-size:1.05rem;font-weight:700;letter-spacing:.08em;color:#2E9E5B;text-transform:uppercase;margin-bottom:6px">🎉 Congratulations! 🎉</div><div style="font-size:1.5rem;font-weight:800;color:#14534F;margin-bottom:4px;font-family:'Fraunces',Georgia,serif">Case Approved</div><div style="width:60px;height:3px;background:linear-gradient(90deg,#2E9E5B,#4CAF8A);border-radius:2px;margin:12px auto 18px"></div><div style="display:inline-block;background:#fff;border:1.5px solid #2E9E5B55;border-radius:20px;padding:5px 18px;font-size:.85rem;color:#2E9E5B;font-weight:700;margin-bottom:18px">${g.crn} · ${g.helpType||"Grant"}</div><p style="font-size:1rem;color:#1a4a38;line-height:1.8;margin:0 0 10px">Dear <strong style="color:#14534F">${g.name}</strong>, your grant request has been <strong style="color:#2E9E5B">officially approved</strong>. Our team will contact you soon. 🤝</p><p style="font-size:.84rem;color:#4a7a62;margin:10px 0 18px;background:#fff8;border-radius:8px;padding:7px 14px;display:inline-block">📅 Applied: <strong>${g.timestamp}</strong> &nbsp;•&nbsp; 💰 Rs. <strong>${amt}</strong></p><div style="font-size:.92rem;font-weight:600;color:#14534F">📞 <a href="tel:+92${window.NGO.alert.replace(/\D/g,'').slice(-10)}" style="color:#14534F;text-decoration:none">${window.NGO.alert}</a> &nbsp;|&nbsp; 📧 <a href="mailto:${window.NGO.email}" style="color:#14534F;text-decoration:none">${window.NGO.email}</a></div></div>`;
              continue;
            }
            else if (d === "rejected") { msg = `Case <strong>${g.crn}</strong> has been Rejected. Please visit our office.<br>📞 <a href="tel:+92${window.NGO.alert.replace(/\D/g,'').slice(-10)}">${window.NGO.alert}</a>`; vibe = "status-red"; icon = "fa-circle-xmark"; }
            else if (s === "closed") { msg = `Case <strong>${g.crn}</strong> Closed. Jazak Allah Khair! 🤲`; vibe = "status-green"; icon = "fa-lock"; }
            else { msg = `Case <strong>${g.crn}</strong> is under process.<br>📞 <a href="tel:+92${window.NGO.alert.replace(/\D/g,'').slice(-10)}">${window.NGO.alert}</a>`; }
            html += `<div class="status-msg ${vibe}" style="margin-bottom:16px"><i class="fa-solid ${icon}"></i><div class="status-title">${g.crn} — ${g.helpType||""}</div><p style="line-height:1.7">${msg}</p><p class="status-note" style="margin-top:10px">📅 Applied: <strong>${g.timestamp}</strong> &nbsp;•&nbsp; 💰 Rs. <strong>${amt}</strong></p></div>`;
          }
          html += `<div class="reg-alert-btns" style="margin-top:8px"><button class="btn btn-primary" onclick="window._gsShowForm()"><i class="fa-solid fa-search"></i> Search Again</button><button class="btn btn-ghost" onclick="window._gsBack()"><i class="fa-solid fa-arrow-left"></i> Back</button></div>`;
          gsShowResult(html);
        })
        .catch(() => {
          setLoading(gsBtn, false);
    ...
    RHS.getStories()
      .then(res => {
        ...
        grid.innerHTML = res.stories.map(s => {
          // Firebase mein: imageUrl, text, category, name, location
          const img      = s.imageUrl  || s.photoURL  || "";
          const story    = s.text      || s.story     || "";
          const category = s.category  || s.helpType  || "Community";
          const name     = s.name      || "";
          const location = s.location  || "Khairpur Tamewali";
          const shortStory = story.length > 160 ? story.substring(0, 160) + "..." : story;

          return `
          <article class="story-card" style="border-radius:16px;overflow:hidden;background:#fff;box-shadow:0 2px 16px rgba(0,0,0,0.08);border:1px solid #E7DFD2;display:flex;flex-direction:column">

            <!-- Image -->
            <div style="position:relative;height:200px;overflow:hidden;flex-shrink:0">
              ${img
                ? `<img src="${img}" alt="${name}" style="width:100%;height:100%;object-fit:cover;display:block" loading="lazy">`
                : `<div style="width:100%;height:100%;background:linear-gradient(135deg,#EEF8F1,#D0F0E0);display:flex;align-items:center;justify-content:center;font-size:3rem">🤲</div>`}
              <!-- Category badge on image -->
              <span style="position:absolute;top:12px;left:12px;background:#E8A33D;color:#fff;font-size:.72rem;font-weight:700;padding:4px 12px;border-radius:20px;letter-spacing:.04em;text-transform:uppercase;font-family:sans-serif">${category}</span>
            </div>
            ...
            <div style="padding:18px 18px 20px;display:flex;flex-direction:column;flex:1">
              ...
              <div style="margin-bottom:10px">
                <h4 style="font-family:'Fraunces',Georgia,serif;font-size:1.1rem;font-weight:700;color:#14534F;margin:0 0 4px">${name}</h4>
                <div style="font-size:.8rem;color:#8A9A96;display:flex;align-items:center;gap:5px;font-family:sans-serif">
                  <i class="fa-solid fa-location-dot" style="color:#4CAF8A;font-size:.75rem"></i>
                  ${location}
                </div>
              </div>

              <!-- Divider -->
              <div style="height:1.5px;background:linear-gradient(90deg,#4CAF8A33,transparent);margin-bottom:12px"></div>

              <!-- Story text -->
              <p style="font-size:.88rem;color:#4A5C58;line-height:1.65;margin:0;flex:1;font-family:sans-serif">${shortStory}</p>
